...
public class TrainMapsUtil {
	...
	public static Double[] getTrainPosition(Double lat, Double lon, Double time, Double lat2, Double lon2, Double vel) {
		...
		/*
		 * if(lat2 < 0) { if(latitudeY < lat2) latitudeY = lat2; } else if(lon2 <
		 * latitudeY) { latitudeY = lat2; }
		 * 
		 * if(lon2 < 0) { if(longitudeX < lon2) longitude*X = lon2; } else if(lon2 <
		 * longitudeX) { longitudeX = lon2; }
		 */
	...
	// vel in mph
	public static Location getTrainPosition(Location currentStop, Location nextStop, long timeC, long timeN,
			double vel) {
		// Double distanceX = lat * longt * Math.sqrt(Time);
	...
	// JOSH FIX
	// public static Double[] getTrainPosition(Double lat, Double lon, Double time,
	// Double lat2, Double lon2, Double vel) {
	// // Double distanceX = lat * longt * Math.sqrt(Time);
	// Double distanceX = lat2 - lat;
	// Double distanceY = lon2 - lon;
	// Double distanceTotal = Math.sqrt(Math.pow(distanceX, 2)+
