...
public class TrainInfo {
	...
	public TrainInfo(final MTAApi api, final String trip_id) throws IOException {
		...
		this.updateStation();
		this.line = new LineInfo(api, this.id.substring(this.id.indexOf("_") + 1, this.id.indexOf("_") + 2)); // Memory
																												// Issue?
		this.setLocation();
	}
	...
	public void setLocation() {
		...
		this.location = TrainMapsUtil.getTrainPosition(currentCoords, nextCoords,
				this.stopTimes.get(1).getArrival().getTime(), this.stopTimes.get(0).getArrival().getTime(), 30.0);
	}

	public String toString() {
		return "Trip ID: " + id + " Current Station: " + currentStation + " Next Station: " + nextStation
				+ " Direction: " + direction + this.location;
	}
	...
	public void updateStation() {
		...
		if (stopTimes != null) {
			...
			this.direction = Direction
					.getDirection(stopTimes.get(0).getStopId().charAt(stopTimes.get(0).getStopId().length() - 1));
		}
	}
}
