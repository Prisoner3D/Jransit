...
/**
 * A builder that lets you set your key once, and then can use that key to create new StopMonitoring or VehicleMonitoring objects in the future.
 * @author kevin
 * @date 8/8/14
 */
public class BusTimeBuilder {
     ...
     * @return true if token is valid and rate is not exceeded, false if token is invalid or rate is exceeded
     * @throws IOException Thrown if the test API call cannot be successfully completed
     */
    public boolean testBusTimeTokenValidity() throws IOException {
        ...
        UrlBuilder builder =
                UrlBuilder.fromString(BusTimeConstants.BASE_STOP_MONITORING_URL.getValue())
                        .addParameter(BusTimeConstants.PARAMETER_MONITORING_REF.getValue(), "308214");
        URL url = builder.toUrl();

        // Gets the JSON for this random bus stop. If the response code is forbidden, then this token is not valid
        try {
