...
public class MTAApi {
	/*
	 * Useful info: Each stop_time_update is a future stop time, past stop times are
	 * omitted. The first StopTime in the sequence is the stop the train is
	 ...
	 * 
	 * A VehiclePosition is in its own FeedMessage. The previous FeedMessage will
	 * contain the StopTimes for that VehiclePosition (Train)
	 * 
	 * Unable to figure out the current position of the train while it is between
	 * stations - Prediction based off previous trip_updates?
	 * 
	 ...
	 * Feed consumers can detect this condition using the timestamp in the
	...
	/**
	 * getArrivalTime(): Gets the arrival time of a specified train at at specified
	 * station within the line
	 * 
	 * @param trip_id
	 *            : The train
	 * @param stop_id
	...
	private FeedMessage getFeed(final TrainFeed id) throws IOException {
		...
		URL url = new URL("http://datamine.mta.info/mta_esi.php?key=" + API_KEY + "&feed_id=" + id); // ID = NUMBERS
																										// ONLY
	...
	/**
	 * getTrains(): Filters out VehiclePosition objects in a line, which represent
	 * trains, and puts them into a list.
	 * 
	...
	/**
	 * getTripIdAndTrain(): Makes a map assigning the trip_id and the
	 * VehiclePosition in the current line
	 * 
	 * @return Map<String, VehiclePosition> : Map of trip_id assigned to
	 *         VehiclePositon
	 */
	...
	/**
	 * getVehiclePosition(): Searches for a VehiclePosition object with the passed
	 * trip_id in the line
	 * 
	 * @param trip_id
	 *            : The term used for search
